...
class SemanticSearchError(RuntimeError):
    """Raised when the search embedder cannot produce query vectors."""
...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    async def embed_texts(self, texts: Sequence[str]) -> list[list[float]]:
        ...
        if len(rows) != len(texts):
            raise SemanticSearchError("Embedding provider returned an unexpected number of vectors.")

        vectors: list[list[float]] = []
        for row in rows:
            embedding = row.get("embedding")
            if not isinstance(embedding, list):
                raise SemanticSearchError("Embedding provider returned an invalid vector payload.")
...
def create_embedding_provider(settings: Settings | None = None) -> EmbeddingProvider:
    ...
    provider = current_settings.embedding_provider.lower().strip()

    if provider in {"hash", "local"}:
        return HashEmbeddingProvider(dimensions=current_settings.embedding_dimensions)
    if provider == "openai":
        if not current_settings.embedding_api_key:
            raise SemanticSearchError("SCIVLY_EMBEDDING_API_KEY must be set for openai embeddings.")
        return OpenAIEmbeddingProvider(
            api_key=current_settings.embedding_api_key,
            api_base=current_settings.embedding_api_base,
    ...
    raise SemanticSearchError(f"Unsupported embedding provider: {current_settings.embedding_provider}")
